        ...
        <aside class="w-72 bg-white border-r border-gray-200 shadow-lg flex-shrink-0">
            <div class="flex flex-col h-screen">
                ...
                <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
                    ...
                    <!-- Dashboard - Toujours visible pour les admins -->
                    @if($isSuperAdmin || $user->hasPermission('view_dashboard'))
                    ...
                    <!-- Plans -->
                    @if($isSuperAdmin || $user->hasPermission('manage_plans'))
                    ...
                    <!-- Abonnements -->
                    @if($isSuperAdmin || $user->hasPermission('manage_subscriptions'))
                    ...
                    @if($isSuperAdmin || $user->hasPermission('manage_payments'))
                    ...
                    <!-- Audios -->
                    @if($isSuperAdmin || $user->hasPermission('manage_audios'))
                    ...
                    <!-- Logs -->
                    @if($isSuperAdmin || $user->hasPermission('view_logs'))
                    ...
                    <!-- Commentaires -->
                    @if($isSuperAdmin || $user->hasPermission('manage_comments'))
                    ...
                    <!-- Messages de Contact -->
                    @if($isSuperAdmin || $user->hasPermission('manage_contact_messages'))
                    ...
                    <!-- Administrateurs - Seulement pour super admin -->
                    @if($isSuperAdmin || $user->hasPermission('manage_admins'))
                    ...
                    @if($isSuperAdmin || $user->hasPermission('manage_payment_providers'))
                    ...
                    <!-- Providers IA -->
                    @if($isSuperAdmin || $user->hasPermission('manage_settings'))
                        <a href="{{ route('admin.ai-providers') }}" class="group flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-all duration-200 {{ request()->routeIs('admin.ai-providers*') ? 'bg-primary-50 text-primary-600 border-l-4 border-primary-600' : 'text-gray-600 hover:bg-gray-50 hover:text-primary-600' }}">
                            <svg class="w-5 h-5 mr-3 flex-shrink-0 {{ request()->routeIs('admin.ai-providers*') ? 'text-primary-600' : 'text-gray-400 group-hover:text-primary-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"></path>
                            </svg>
                            <span class="font-medium">Providers IA</span>
                        </a>
                    @endif

                    <!-- Paramètres - Seulement pour super admin -->
                    @if($isSuperAdmin)
